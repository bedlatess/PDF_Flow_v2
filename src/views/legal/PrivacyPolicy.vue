...
const content: Record<'zh' | 'en', LegalContent> = {
  zh: {
    ...
    description:
      'PDF-Flow 是面向日常文档处理的在线 PDF 工具。我们尽量让基础操作在浏览器本地完成；当你主动使用 OCR、Office 转 PDF、AI 分析等云端能力时，相关文件才会被上传并用于完成该次任务。',
    summaryTitle: '简短说明',
    summary:
      '我们不会出售你的个人信息，也不会为了广告画像读取你的文件内容。上传到云端处理的文件仅用于完成你发起的任务、排查异常和保障服务安全，并会尽量缩短保留时间。',
    note:
      '本政策说明 PDF-Flow 当前的数据处理方式。随着支付、团队协作或企业能力完善，我们会同步更新本页面。',
    sections: [
      {
        icon: Database,
        title: '1. 我们收集的信息',
        paragraphs: [
          '当你访问网站时，我们可能接收基础访问数据，例如浏览器类型、访问时间、页面路径、语言设置和必要的安全日志。这些信息用于保持网站稳定运行、排查异常访问和改进体验。',
          '当你注册或登录账户时，我们会保存邮箱、显示名称、加密后的密码、账户角色、订阅状态、登录时间等必要信息。我们不会保存你的明文密码。',
        ],
        bullets: [
          '账户信息：邮箱、显示名称、认证状态、角色权限。',
          '运行信息：任务状态、错误编号、必要诊断日志。',
          '支付信息：如启用订阅，付款细节由支付服务商处理，PDF-Flow 仅保存必要的订阅状态。',
        ],
      },
      {
        icon: FileLock2,
        ...
        paragraphs: [
          ...
          'OCR 文字识别、Office 转 PDF、AI PDF 分析以及部分高级能力需要服务端参与。你主动提交这些任务时，相关文件会被上传并在服务器上临时处理。',
        ],
        bullets: [
          '我们只会为了完成你选择的功能处理文件。',
      ...
      },
      {
        ...
        title: '3. 我们如何使用信息',
        paragraphs: [
          '我们使用必要信息提供账户登录、文件处理、任务进度、结果下载、权限控制、订阅识别、异常排查和安全防护。',
          '如果你提交问题反馈，我们可能根据你提供的截图、时间点、错误编号和操作步骤定位问题。请不要在反馈中主动发送不必要的敏感文件。',
        ],
      },
      {
        icon: Trash2,
      ...
      },
      {
        ...
        paragraphs: [
          ...
          '你可以请求查看、更正或删除账户相关信息。某些安全、支付、审计或故障排查记录可能需要按合理期限继续保留。',
        ],
      },
      {
        icon: ShieldCheck,
      ...
      },
      {
        ...
        title: '7. 政策更新',
        paragraphs: [
          '随着支付、团队协作、企业 API 或更多云端功能上线，本政策可能更新。重要变化会尽量通过页面提示、版本记录或站内说明告知。',
        ],
      },
      {
        icon: Mail,
        title: '8. 联系我们',
        paragraphs: [
          '如果你对隐私、文件处理或账户数据有疑问，请通过网站展示的支持渠道联系我们。反馈问题时，建议提供页面截图、时间点、错误编号和简短复现步骤。',
        ],
      },
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          'OCR, Office to PDF, AI PDF analysis, and some advanced workflows require backend processing. When you submit these tasks, related files are uploaded temporarily.',
        ],
      },
      {
        icon: ServerCog,
