...
- **2026-06-10 Frontend deploy cache hardening**: identified the remaining “login/register sometimes render blank after deploy” risk as a stale service-worker cache path rather than the auth views themselves. `index.html` now proactively unregisters legacy `/sw.js` workers, clears `pdf-flow-*` caches, and forces one reload after cleanup so old cached chunks do not mix with newly deployed auth pages. In the same pass, `OCRPDF.vue`, `OfficeToPDF.vue`, `FillFormPDF.vue`, and `AnnotatePDF.vue` were tightened toward the AI-analyzer layout by moving each page closer to the same hero -> notice -> access -> upload/workspace rhythm. Local verification on 2026-06-10: `npm run build` passed and Playwright route checks confirmed `/auth/login`, `/auth/register`, `/tools/ocr`, `/tools/office-to-pdf`, `/tools/fill-form`, and `/tools/annotate` render without runtime errors.
